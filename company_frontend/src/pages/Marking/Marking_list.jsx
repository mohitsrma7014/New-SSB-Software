...
import ForgingForm from "./Marking_edit_form";
...
const Marking_list = () => {
  ...
  const fetchForgings = async () => {
    try {
      ...
      const response = await axios.get("http://192.168.1.199:8001/marking/MarkingViewSet/");
  ...
  const deleteForging = async (id) => {
    try {
      await axios.delete(`http://192.168.1.199:8001/marking/MarkingViewSet/${id}/`);
  ...
  return (
    <div className="w-full p-6 mt-20" >
      <Sidebar />
      <h1 className="text-3xl font-bold mb-6">Marking Records</h1>
