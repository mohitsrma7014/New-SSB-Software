...
const Shot_blast_form = () => {
  ...
  const lineOptions = ["Jitendra", "Ashok", "Chetram","Kanhaiya", "Inder", "Sona"]; // Dropdown options for line
  ...
  const fetchPartDetails = async (batch_number, index) => {
    try {
      ...
      const targetDetailsResponse = await axios.get(
        "http://192.168.1.199:8001/shot_blast/get-target-details5/",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(
        "http://192.168.1.199:8001/shot_blast/api/sb/bulk-add/",
        dataToSubmit
      );
  ...
  return (
    <div className="w-full p-6 bg-white shadow-md rounded-lg mt-20">
      <Sidebar /> 
      <h1 className="text-2xl font-semibold text-gray-700 mb-6">Add Shot-Blast Data</h1>
      <form onSubmit={handleSubmit}>
        ...
        <div className="overflow-x-auto mb-6"> {/* Make the table horizontally scrollable */}
          <table className="table-auto border-collapse min-w-full table-layout-auto">
            <thead>
              <tr className="bg-gray-100">
                ...
                <th className="min-w-[90px] text-left text-sm text-gray-600">Machine no.</th>
                <th className="min-w-[90px] text-left text-sm text-gray-600">Ring Weight</th>
                <th className="min-w-[90px] text-left text-sm text-gray-600">Pcs.</th>
                    ...
                    <td key={field} className="">
                      {field === "operator1"  ? (
                        ...
                        >
                          ...
                          {(field === "operator1" ? lineOptions : field === "line_incharge" ? lineInchargeOptions : formanOptions).map((option) => (
                            <option key={option} value={option}>{option}</option>
                          ))}
                        </select>
                      ) : (
...
export default Shot_blast_form;