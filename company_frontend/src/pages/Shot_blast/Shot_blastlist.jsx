...
import ForgingForm from "./Shot_blast_edit_form";
...
const Pre_mclist = () => {
  ...
  const fetchForgings = async () => {
    try {
      ...
      const response = await axios.get("http://192.168.1.199:8001/shot_blast/ShotblastViewSet/");
  ...
  const deleteForging = async (id) => {
    try {
      await axios.delete(`http://192.168.1.199:8001/shot_blast/ShotblastViewSet/${id}/`);
  ...
  return (
    <div className="w-full p-6 mt-20" >
      <Sidebar />
      <h1 className="text-3xl font-bold mb-6">Shot-Blast Records</h1>
