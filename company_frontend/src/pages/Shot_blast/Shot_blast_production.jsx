...
import './Shot_blast_production.css';
import debounce from 'lodash.debounce';

const Shot_blast_production = () => {
  ...
  const selectedColumns = ['date','shift','batch_number', 'component', 'weight','machine', 'no_of_pic','operator','verified_by'];
  ...
  const fetchData = async () => {
    if (date) {
      ...
      try {
        const response = await axios.get('http://192.168.1.199:8001/shot_blast/api/production-report', { params });
  ...
  const fetchSuggestions = async (type, value) => {
    ...
    try {
      const response = await axios.get('http://192.168.1.199:8001/shot_blast/api/suggestions', {
        params: { type, value },
      });
  ...
  const customerChartOptions = {
    ...
    xAxis: {
      categories: sortDataDescending(data.customer_data || []).map(item => item.machine),
      labels: {
        rotation: 0,
        staggerLines: 2,
        style: {
  ...
  const lineChartOptions = {
    ...
    xAxis: {
      categories: sortDataDescending1(data.line_data || []).map(item => item.operator),
      labels: {
        rotation: 0,
        staggerLines: 2,
        style: {
    ...
    series: [
      {
        name: 'Production Weight (Ton)',
        data: sortDataDescending1(data.line_data || []).map(item => item.production_weight_ton),
        color: '#ffca28',
        borderRadius: 5,
        dataLabels: {
          enabled: true,
  ...
  return (
    <div className="app">
      <h1>Shot Blast Production Dashboard</h1>
      {/* Filters */}
...
export default Shot_blast_production;
