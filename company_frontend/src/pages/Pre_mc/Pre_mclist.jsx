...
import ForgingForm from "./Pre_mc_edit_form";
...
const Pre_mclist = () => {
  ...
  const fetchForgings = async () => {
    try {
      ...
      const response = await axios.get("http://192.168.1.199:8001/pre_mc/Pre_Mc_ViewSet/");
  ...
  const deleteForging = async (id) => {
    try {
      await axios.delete(`http://192.168.1.199:8001/pre_mc/Pre_Mc_ViewSet/${id}/`);
  ...
  return (
    <div className="w-full p-6 mt-20" >
      <Sidebar />
      <h1 className="text-3xl font-bold mb-6">Pre M/C Records</h1>
