...
import ForgingForm from "./Fi_edit_form";
...
const Fi_list = () => {
  ...
  const fetchForgings = async () => {
    try {
      ...
      const response = await axios.get("http://192.168.1.199:8001/fi/FiViewSet/");
  ...
  const deleteForging = async (id) => {
    try {
      await axios.delete(`http://192.168.1.199:8001/fi/FiViewSet/${id}/`);
  ...
  return (
    <div className="w-full p-6 mt-20" >
      <Sidebar />
      <h1 className="text-3xl font-bold mb-6">Final-Inspection Records</h1>
