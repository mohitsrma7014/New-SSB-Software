...
const BulkAddFormfi = () => {
  ...
  const fetchPartDetails = async (batch_number, index) => {
    try {
      ...
      const targetDetailsResponse = await axios.get(
        "http://192.168.1.199:8001/fi/get-target-details6/",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(
        "http://192.168.1.199:8001/fi/api/fi/bulk-add/",
        dataToSubmit
      );
  ...
  return (
      <div className="w-full p-6 bg-white shadow-md rounded-lg mt-20">
        <Sidebar /> 
        <h1 className="text-2xl font-semibold text-gray-700 mb-6">Add Fi Data</h1>
            ...
            <table className="table-auto border-collapse min-w-full table-layout-auto">
              <thead>
                <tr className="bg-gray-100">
                  ...
                  <th className="min-w-[75px] text-left text-sm text-gray-600">Rust</th>
                    ...
                    {Object.keys(row).map((field) => (
                      <td key={field} className="">
                        {field === "jk"   ? (
                          <select
                            value={row[field]}
                            disabled={[ "customer", "heat_number", "rm_grade", "total_production", "target"].includes(field)}
