...
const AnalyticsPage = () => {
  ...
  const selectedColumns = ['date','shift','batch_number', 'component', 'slug_weight', 'production','up_setting','half_piercing','full_piercing','ring_rolling','sizing','overheat','bar_crack_pcs','verified_by'];
  ...
  const fetchData = async () => {
    if (date) {
      ...
      try {
        const response = await axios.get('http://192.168.1.199:8001/forging/api/production-report', { params });
  ...
  const customerChartOptions = {
    ...
    xAxis: {
      categories: sortDataDescending(data.customer_data || []).map(item => item.normalized_customer),
      labels: {
        rotation: 0,
        staggerLines: 2,
        style: {
  ...
  return (
    <div className="app">
      <h1>Forging Production Dashboard</h1>
      {/* Filters */}
