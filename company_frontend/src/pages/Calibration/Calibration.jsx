...
const Calibration = () => {
  ...
  return (
    <div className="w-full h-screen flex flex-col p-0 bg-white mt-24">
      <Sidebar />
      <h2 className="text-2xl font-bold text-center mb-6">Calibration Management</h2>
      <div className="fixed top-30 right-7 z-50">
        ...
        <table className="w-full border-collapse border rounded-lg shadow-md">
          <thead className="bg-gray-200 text-black sticky top-0 z-10">
            <tr>
              ...
              <th className="px-2 py-1">Serial Number</th>
          ...
          <tbody>
            {filteredComplaints.map((complaint) => (
              <tr key={complaint.id} className="border-t">
                ...
                <td className="px-2 py-2 text-center">{complaint.serial_number}</td>
                ...
                <td className={`px-2 py-2 text-center font-bold ${complaint.status === "closed" ? "text-red-500" : "text-green-500"}`}>{complaint.status}</td>
