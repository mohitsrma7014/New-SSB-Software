...
const cnc_production = () => {
  ...
  const selectedColumns = ['date','shift','batch_number', 'component', 'machine_no', 'mc_type','operator','inspector','setup','target','production','remark','total_cnc_rejection','total_forging_rejection','total_pre_mc_rejection','total_rejection','total_rework','verified_by'];
  ...
  const fetchData = async () => {
    ...
    if (date) {
      ...
      try {
        const response = await axios.get('http://192.168.1.199:8001/cnc/api/production-report', { params });
  ...
  const fetchSuggestions = async (type, value) => {
    ...
    try {
      const response = await axios.get('http://192.168.1.199:8001/cnc/api/suggestions', {
        params: { type, value },
      });
  ...
  const customerChartOptions = {
    ...
    xAxis: {
      categories: sortDataDescending(data.customer_data || []).map(item => item.operator),
      labels: {
        rotation: 0,
        staggerLines: 2,
        style: {
  ...
  const lineChartOptions = {
    ...
    xAxis: {
      categories: sortDataDescending(data.line_data || []).map(item => item.line),
      labels: {
        rotation: 0,
        staggerLines: 2,
        style: {
  ...
  return (
    <div className="app">
      <h1>CNC Production Dashboard</h1>
      {/* Filters */}
    ...
    <div className="loading-section">
        ...
        <>
          ...
          <div className="kpi-row">
            ...
            <div className="kpi">Total Rework (Kg) : {data.production_weight_kg}</div>
