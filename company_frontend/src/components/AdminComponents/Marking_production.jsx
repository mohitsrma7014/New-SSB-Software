...
const Marking_production = () => {
  ...
  const selectedColumns = ['date','shift','batch_number', 'component', 'machine', 'qty','operator','verified_by'];
  ...
  const fetchData = async () => {
    if (date) {
      ...
      try {
        const response = await axios.get('http://192.168.1.199:8001/marking/api/production-report', { params });
  ...
  const fetchSuggestions = async (type, value) => {
    ...
    try {
      const response = await axios.get('http://192.168.1.199:8001/marking/api/suggestions', {
        params: { type, value },
      });
  ...
  const lineChartOptions = {
    ...
    xAxis: {
      categories: sortDataDescending(data.line_data || []).map(item => item.operator),
      labels: {
        rotation: 0,
        staggerLines: 2,
        style: {
  ...
  return (
    <div className="app">
      <h1>Marking Production Dashboard</h1>
      {/* Filters */}
...
export default Marking_production;
