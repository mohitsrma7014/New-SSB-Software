...
const Pre_mc_production = () => {
  ...
  const selectedColumns = ['date','batch_number', 'component', 'customer', 'qty','shop_floor','verified_by'];
  ...
  const fetchData = async () => {
    if (date) {
      ...
      try {
        const response = await axios.get('http://192.168.1.199:8001/pre_mc/api/production-report', { params });
  ...
  const fetchSuggestions = async (type, value) => {
    ...
    try {
      const response = await axios.get('http://192.168.1.199:8001/pre_mc/api/suggestions', {
        params: { type, value },
      });
  ...
  const customerChartOptions = {
    ...
    xAxis: {
      categories: sortDataDescending(data.customer_data || []).map(item => item.customer),
      labels: {
        rotation: 0,
        staggerLines: 2,
        style: {
  ...
  const lineChartOptions = {
    ...
    xAxis: {
      categories: sortDataDescending(data.line_data || []).map(item => item.shop_floor),
      labels: {
        rotation: 0,
        staggerLines: 2,
        style: {
  ...
  return (
    <div className="app">
      <h1>Pre Machining Production Dashboard</h1>
      {/* Filters */}
...
export default Pre_mc_production;
