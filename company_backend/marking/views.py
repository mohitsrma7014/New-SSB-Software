...
class ProductionReport(APIView):
    def get(self, request, *args, **kwargs):
        ...
        operator = request.query_params.get('line')
        ...
        # Apply filters
        report = marking.objects.filter(filters)
        ...
        customer_data = [{'machine': item['machine'], 'production_weight_ton': item['total_production_weight_kg'] } for item in customer_data]
        line_data = [{'operator': item['operator'], 'production_weight_ton': item['total_production_weight_kg'] } for item in line_data]
...
class SuggestionView(APIView):
    def get(self, request):
        ...
        if search_type == 'line':
            ...
            suggestions = marking.objects.filter(machine__icontains=search_query).values('machine').distinct()
            ...
            suggestions = marking.objects.filter(batch_number__icontains=search_query).values('batch_number').distinct()
            ...
            suggestions = marking.objects.filter(component__icontains=search_query).values('component').distinct()
...
def balance_for_target_autofill():
    ...
    batch_tracking = marking.objects.all()
...
def get_target_details3(request):
...
from .serializers import MarkingSerializer
...
class BulkAddmarkingAPIView(APIView):
    """
    ...
    def post(self, request, *args, **kwargs):
        ...
        for i, entry in enumerate(data):
            ...
            required_fields = [
                "batch_number", "date", "shift", "component","heat_no", "machine", "operator", "target", "total_produced", "qty", "verified_by",
            ]
        ...
        try:
            with transaction.atomic():
                ...
                marking.objects.bulk_create(valid_entries)
