...
class ProductionReport(APIView):
    def get(self, request, *args, **kwargs):
        ...
        # Apply filters
        report = Visual.objects.filter(filters)
...
class SuggestionView(APIView):
    def get(self, request):
        ...
        if search_type == 'line':
            ...
            suggestions = Visual.objects.filter(line__icontains=search_query).values('line').distinct()
            ...
            suggestions = Visual.objects.filter(chaker__icontains=search_query).values('chaker').distinct()
            ...
            suggestions = Visual.objects.filter(batch_number__icontains=search_query).values('batch_number').distinct()
            ...
            suggestions = Visual.objects.filter(component__icontains=search_query).values('component').distinct()
...
def balance_for_target_autofill():
    ...
    batch_tracking = Visual.objects.all()
    ...
    df_bt_extracted = df_bt[['batch_number', 'cnc_height', 'cnc_od', 'cnc_bore', 'cnc_groove', 
                  'cnc_dent','cnc_rust', 'forging_height', 'forging_od', 'forging_bore', 'forging_crack', 
                  'forging_dent', 'pre_mc_height', 'pre_mc_od', 'pre_mc_bore','marking','production']]
    ...
    df_bt_extracted['total_production'] = df_bt_extracted[[  'cnc_height', 'cnc_od', 'cnc_bore', 'cnc_groove', 
                  'cnc_dent','cnc_rust', 'forging_height', 'forging_od', 'forging_bore', 'forging_crack', 
                  'forging_dent', 'pre_mc_height', 'pre_mc_od', 'pre_mc_bore','marking','production']].sum(axis=1)
...
def get_target_details8(request):
...
class BulkAddVisualAPIView(APIView):
    """
    ...
    def post(self, request, *args, **kwargs):
        ...
        for i, entry in enumerate(data):
            ...
            required_fields = [
                "batch_number", "date", "shift", "component","target1", "target","chaker","chaker1","production", "remark", "cnc_height",
                ...
                "total_produced", "verified_by","marking"
            ]
        ...
        try:
            with transaction.atomic():
                ...
                Visual.objects.bulk_create(valid_entries)
